...
<div class="flex gap-5 px-5 h-[85%] justify-evenly relative mt-3">
    <div class="list-checkout-wrapper border border-black rounded-lg w-[50%] h-full relative">
        <div class="btn-group flex gap-3 bg-slate-300 rounded-lg p-3 absolute top-0 left-0 z-10">
            <a href="{{ route('transaction_page') }}" class="px-3 py-1 bg-green-500 text-white rounded-md border-2 border-slate-300 text-sm flex items-center"><i class="fa fa-plus mr-2"></i> Tambah</a>
            ...
            <a href="{{ route('cancel_transaction') }}" onclick="return confirm('apakah yakin ingin membatalkan transaksi?')" class="px-3 py-1 bg-red-500 text-white rounded-md border-2 border-slate-300 text-sm flex items-center"><i class="fa fa-angle-double-left mr-2"></i> Batal</a>
            ...
            @foreach ($selling_unit as $product)
                <div class="item flex justify-between p-3 border-slate-600">
                    <div>
                        ...
                        <option value="{{ $selling_units->unit->name }}">{{ $selling_units->unit->name}}<i class="fa fa-chevron-down ml-2"></i></option>   
                        @endforeach    
                        </select>
                    </div>
