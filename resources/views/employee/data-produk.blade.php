            ...
            <table class="w-full text-md border-collapse rounded-corners rounded-t-xl overflow-auto no-scrollbar"
                ...
                <thead>
                    <tr class=" bg-[#4B4B4B] text-white font-semibold">
                        ...
                        <td class=" border border-l-0 border-t-0 border-r-0 border-black capitalize w-[25%] text-center">
                            catatan obat</td>
                    </tr>
                </thead>
                <tbody>
                        ...
                        <tr class="">
                            ...
                            <td class=" border border-l-0 border-t-0 border-black font-medium">
                                <div class="btn-group mx-auto w-fit text-white text-sm flex gap-3">
                                    <a href="{{ route('add_selling_unit', $item->id) }}"
                                        class="w-[25px] h-[25px] p-1 bg-green-500 flex justify-center items-center"><i
                                            class="fa fa-archive" aria-hidden="true"></i></a>
                                    <a href="{{ route('update_data_product', $item->id) }}"
                                        class="w-[25px] h-[25px] p-1 bg-green-500 flex justify-center items-center"><i
                                            class="bi bi-pencil-square"></i></a>
                                    <a href="{{ route('table_selling_unit', $item->id) }}"
                                        class="w-[25px] h-[25px] p-1 bg-blue-500 flex justify-center items-center"><i
                                            class="fa fa-exclamation-circle" aria-hidden="true"></i></a>
                                    <form action="{{ route('delete_data_product', $item->id) }}" method="POST">
                                        @csrf
                            ...
                            <td class=" border border-l-0 border-t-0 border-black font-medium">{{ $item->product_name }}
                            </td>
                            ...
                            <td class=" border border-l-0 border-t-0 border-black text-center font-light">
                                {{ $item->smallest_selling_unit }}</td>
                            <td class=" border border-l-0 border-t-0 border-black font-semibold">{{ $item->catatan_obat }}
                            </td>
                        </tr>
                    @endforeach
                </tbody>
